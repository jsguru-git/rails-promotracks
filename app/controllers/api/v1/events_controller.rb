class Api::V1::EventsController < Api::V1::ApiApplicationController
  ...
  def update
    ...
    unless user_event_params[:check_in].nil?
      ...
      if current_user.events.includes(:user_events).where(:user_events => {:user_id => current_user.id, :status => UserEvent::statuses[:accepted], :check_out => nil}).where.not(:user_events => {:check_in => nil}).size>0
